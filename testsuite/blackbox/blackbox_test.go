...
func TestProjectsAndGroups(t *testing.T) {
	...
	if len(projectView["members"].([]any)) == 0 {
		t.Fatalf("expected project members: %v", projectView)
	}
	groupView := mustJSON(t, runMissis(t, store, "show", "--json", "--group", "eng"))
	if len(groupView["members"].([]any)) == 0 {
		t.Fatalf("expected group members: %v", groupView)
	}
