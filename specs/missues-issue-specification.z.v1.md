...
**Primary content unit:** addressable part  
...
- [Addressable parts and references](#6-addressable-parts-instead-of-one-large-body)
...
PART    = a small addressable unit of ticket information
...
5. **Make every useful information unit addressable.**
   A ticket body must not be the only writable unit. Sections, items, evidence, code references, and verification results must be independently addressable.
...
A **part** is the smallest logical unit that should be independently readable, writable, linkable, searchable, versioned, or verified.
...
A part has a stable logical address. Its value changes through events; the address itself remains stable unless explicitly superseded or aliased.

## 6.2 Conventional sections
...
## 6.3 Nested and list-like parts

Parts may contain named children:
...
issue set 184/evidence/production-log \
  "Observed enqueue 4 ms after cancellation"
```

`issue show 184/evidence` returns all children without requiring a separate list command.

## 6.4 Part value kinds
...
## 6.5 Open-world content with closed-world constraints
...
```text
Specific ticket types may require, constrain, or interpret selected parts.
```
...
The system MUST NOT discard unrecognized headings or fields merely because an ontology does not understand them.
...
Human references are aliases. Internally, entities SHOULD use globally unique immutable identifiers such as UUIDs or ULIDs.

Example mapping:

```text
safedesign#184
    → ticket:01J5.../part/problem
```

Renaming a project or changing a human ticket number MUST NOT change the canonical identity.

## 7.4 Part path syntax
...
- H2 and deeper headings become nested part paths;
- heading text is normalized to a stable slug;
- explicit IDs override generated slugs;
...
type Ref struct {
    ...
    Path   []string // optional logical part path
}
```
...
A `Part` is a projection. Its changes are stored as events.
...
- Markdown parser;
- heading-to-part mapping;
- source spans;
- optional annotations and front matter;
- atomic batch import;
- Markdown export.
...
- [ ] `set` covers scalar updates, nested parts, additions, links, code refs, Git refs, retractions, and corrections.

### 29.2 Parts and Markdown

- [ ] Tickets support arbitrary addressable parts and nested items.
- [ ] A large Markdown document can create or update a ticket.
- [ ] Incremental part updates produce the same internal model as Markdown import.
- [ ] Imported parts preserve source artifact and line-span provenance.
- [ ] Unrecognized Markdown content is never silently discarded.
- [ ] Markdown export preserves part hierarchy and references.
...
- [ ] Ticket types may define required parts, legal links, obligations, and completion contracts.
...
- [ ] Plugins can pre-process and post-process any relevant part or event phase.
...
- [ ] Hook cycles are detected and stopped.
...
- [ ] Parts, not only whole tickets, are search documents.
...
- [ ] Conflicting scalar updates support expected-revision checks.
...
Input: title, Markdown, part update, link      │
            ...
            └───────┬────────┘       └─────────┬─────────┘
                    │                           │
                    ▼                           ▼
             proposed parts, links, events, evidence
                                │
                                ▼
                     ┌────────────────────┐
                     │ Immutable event log│
