...
1. Run `missis --agent-brief` before ticket work (or `missis --agent-brief --json` for machine output). It prints the exact new/show/set syntax and the rules. For the active project/group/focus, run `missis show --context`. Trust its output over memory.
